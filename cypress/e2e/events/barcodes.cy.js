describe("Verify purchased tickets by ", () => {
  ...
  it("ensuring a single barcode gets issued for all tickets within an order-TA-68", function () {
    ...
    cy.getChakraSkeletonItem().contains(uniqueEventName).click({ force: true });
    cy.url().should("contain", uniqueEventName);
    ...
    cy.get('div[data-testid="invoice-barcode-items"]').should("have.length", 1);
  });
});
